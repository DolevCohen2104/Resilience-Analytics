...
const SimulationManagement: React.FC = () => {
  ...
  const Stars = ({ count }: { count: number }) => (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        i <= count
          ? <RiStarFill key={i} className="text-idf-orange text-xs" />
          : <RiStarLine key={i} className="text-dark-border text-xs" />
      ))}
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">ניהול סימולציות</h1>
      </div>
              ...
              {(['all', 'combat', 'command', 'institutional'] as const).map(s => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    filterSector === s ? 'bg-idf-blue/10 text-idf-blue' : 'bg-dark-surface text-text-dim hover:text-text-secondary'
                  }`}
                >
              ...
              {[1, 2, 3, 4, 5].map(d => (
                <button
                  ...
                  className={`w-7 h-7 rounded-lg text-xs font-mono font-bold transition-colors ${
                    filterDifficulty === d ? 'bg-idf-orange/10 text-idf-orange' : 'bg-dark-surface text-text-dim hover:text-text-secondary'
                  }`}
                >
                  {d}
                </button>
          ...
          <div className="grid grid-cols-3 gap-4">
            {filteredScenarios.map(scenario => (
              <div key={scenario.id} className="stat-card group">
                ...
                <div className="h-32 bg-gradient-to-br from-dark-bg to-dark-hover rounded-lg mb-4 flex items-center justify-center border border-dark-border">
                  <RiGamepadLine className="text-3xl text-text-dim/30" />
                </div>

                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3 className="text-sm font-semibold">{scenario.nameHe}</h3>
                    <p className="text-[10px] text-text-dim">{scenario.name}</p>
                  </div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs text-text-dim">{scenario.avgDuration}</span>
                </div>

                <div className="flex items-center justify-between text-xs text-text-dim mb-4">
                  <span>{scenario.completedBy} צוערים השלימו</span>
                  <span>ממוצע: <strong className="text-text-primary font-mono">{scenario.avgPerformance}%</strong></span>
                </div>

                <button className="w-full flex items-center justify-center gap-2 bg-idf-blue/10 text-idf-blue border border-idf-blue/20 rounded-lg py-2 text-xs font-medium hover:bg-idf-blue/20 transition-colors">
      ...
      {activeSection === 'tree' && (
        <div className="bg-dark-surface border border-dark-border rounded-[14px] p-6">
          <h2 className="text-base font-semibold mb-6">עורך עץ החלטות - היתקלות בשטח בנוי</h2>
          ...
          <div className="relative min-h-[400px]">
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: -1 }}>
              <line x1="50%" y1="60" x2="30%" y2="100" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="50%" y1="60" x2="70%" y2="100" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="30%" y1="140" x2="15%" y2="180" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="30%" y1="140" x2="45%" y2="180" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="70%" y1="140" x2="75%" y2="180" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="15%" y1="220" x2="30%" y2="260" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="75%" y1="220" x2="60%" y2="260" stroke="#1E293B" strokeWidth="1.5" />
              <line x1="45%" y1="220" x2="60%" y2="260" stroke="#1E293B" strokeWidth="1.5" />
            </svg>
          </div>
          ...
          <div className="mt-8 border-t border-dark-border pt-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-semibold">הגדרות ביו-אדפטיביות</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={setting.label} className="flex items-center justify-between p-3 bg-dark-bg rounded-lg">
                  <span className="text-xs text-text-secondary">{setting.label}</span>
                  <span className={`text-xs font-mono font-bold ${setting.color}`}>{setting.value}</span>
                </div>
              ))}
            </div>
            <div className="mt-3 flex items-center gap-3 p-3 bg-idf-red/5 border border-idf-red/10 rounded-lg">
              ...
              <span className="text-xs text-text-primary">בדיקת שבירה (Breaking Point Test) - מאפשרת למנוע להקשיח ללא הקלה</span>
            </div>
          </div>
        </div>
      )}
      ...
      {activeSection === 'live' && (
        <div className="grid grid-cols-5 gap-4">
          {/* Simulation Viewport */}
          <div className="col-span-3 bg-dark-surface border border-dark-border rounded-[14px] p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold">שידור חי - היתקלות בשטח בנוי</h2>
              <div className="flex items-center gap-2">
                <div className="glow-dot bg-idf-green shadow-[0_0_8px_rgba(0,229,160,0.5)]" />
                <span className="text-[10px] text-idf-green font-medium">LIVE</span>
              </div>
            </div>
            <div className="aspect-video bg-gradient-to-br from-dark-bg to-dark-hover rounded-lg flex items-center justify-center border border-dark-border">
              <div className="text-center">
                <RiEyeLine className="text-4xl text-text-dim/20 mx-auto mb-2" />
                <p className="text-xs text-text-dim">Unreal Engine Pixel Streaming Viewport</p>
                <p className="text-[10px] text-text-dim/50">Simulation feed would appear here</p>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-2 space-y-3">
            <div className="bg-dark-surface border border-dark-border rounded-[14px] p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold">ביומטרי בזמן אמת</h3>
              ...
              {[
                ...
                <div key={metric.label} className="mb-4">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold font-mono" style={{ color: metric.color }}>{metric.value} {metric.unit}</span>
                  </div>
            ...
            <div className="bg-dark-surface border border-dark-border rounded-[14px] p-4">
              <h3 className="text-sm font-semibold mb-3">מצב מנוע</h3>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={state.label} className={`p-2 rounded-lg text-center text-xs font-medium ${
                    state.active ? 'bg-idf-green/10 text-idf-green border border-idf-green/20' : 'bg-dark-bg text-text-dim'
                  }`}>
                    {state.label}
                  </div>
                ))}
            ...
            <button className="w-full bg-idf-red/10 text-idf-red border border-idf-red/20 rounded-xl py-3 text-sm font-semibold hover:bg-idf-red/20 transition-colors">
